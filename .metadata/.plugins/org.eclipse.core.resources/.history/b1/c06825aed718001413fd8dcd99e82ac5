...
public class ReservationDB {
    ...
    public ArrayList<Reservation> getreservations() {
        String sql = "SELECT RESERV_ID,G_ID,R_ID,TO_CHAR(RESERV_DATE,'DD/MM/YY'),RESERV_STATUS,RESERV_NIGHT "
        ...
        try (Connection connection = getConnection();
        	...
        	while(rs.next()) {
				...
				String roomdate = rs.getString("TO_CHAR(RESERV_DATE,'DD/MM/YY')");
	...
	public Reservation getreservation(int rid) {
	 ...
	 public boolean addreservation(Reservation reservation) {
        String sql ="INSERT INTO RESERVATION (RESERV_ID,G_ID,R_ID,RESERV_DATE,RESERV_STATUS,RESERV_NIGHT) " +
            "VALUES (rev_seq.nextval,?,?,TO_DATE(?,'DD/MM/YY'),?,?)";
        try (Connection connection = getConnection();
             PreparedStatement ps = connection.prepareStatement(sql)) {	
   			ps.setInt(1, reservation.getguest().getid());
            ps.setInt(2, reservation.getroom().getrid());
				ps.setString(3,reservation.getreservdate());
				ps.setInt(4, reservation.getstatus());
				ps.setInt(5,reservation.getnumnights());	
            ps.executeUpdate();
            connection.close();
            return true;
        }
